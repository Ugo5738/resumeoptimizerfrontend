...
const Signup: React.FC<SignupProps> = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data) {
        navigate("/email-verification", { state: { message: "Registration successful. Please check your email to verify your account." } });
      }
    } catch (error) {
      if (isAxiosError(error) && error.response) {
        ...
        if (errorData.errors) {
          ...
          setErrors({ email: "An error occurred during signup. Please check your information and try again." });
        }
      } else {
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    try {
      ...
      if (response.data.access) {
        ...
        setErrors({ general: "Google signup successful, but no access token received. Please try logging in."});
        navigate("/login");
      }
    } catch (error) {
      console.error("Google signup error:", error);
      if (isAxiosError(error) && error.response) {
        setErrors({ general: `Google signup failed: ${error.response.data.error || 'Unknown error'}`});
      } else {
        setErrors({ general: "An unexpected error occurred during Google signup. Please try again."});
      }
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center overflow-hidden">
      ...
      <div className="w-full max-w-md p-8 bg-white bg-opacity-90 rounded-lg shadow-xl z-10">
        <h2 className="text-3xl font-bold mb-6 text-center text-gray-800">Create Free Account</h2>
